import ZegoUIKit, { DefaultExpressEventHandler, DefaultZIMEventHandler, SingletonScope, ZegoUIKitScenario, ZegoUIKitSignaling, ZegoUIKitUser, getSingletonInstance, makeListenerID, deleteSingletonInstance, zlogerror, zloginfo, CallInvitationRejectReason, zlogwarning, getAppState, ZegoInvitationType, UIKitReport, } from "@/uni_modules/zego-UIKitCore";
import { defalutPrebuiltCallInvitationConfig } from "../../config/ZegoUIKitPrebuiltCallInvitationConfig";
...
export class CallInvitationServiceImpl {
    static name = '_CallInvitationServiceImpl';
    getVersion() {
    ...
    zimEventHandler = new DefaultZIMEventHandler({});
    invitationListener = {
        onInvitationReceived: function (inviter, type, data) {
            ...
            const _invitees = invitees.map((i) => ({
                userID: i.user_id,
        ...
        onInvitationTimeout: function (inviter, data) {
        ...
        onInvitationResponseTimeout: function (invitees, data) {
            ...
            const { roomID, invitationID } = data;
            const _invitees = invitees.map((user) => ({
                userID: user.userID,
        ...
        onInvitationAccepted: function (invitee, data) {
        ...
        onInvitationRefused: function (invitee, data) {
        ...
        onInvitationCanceled: function (inviter, data) {
    ...
    async init(appID, appSign) {
        ...
        if (result) {
            ...
            try {
                ...
                ZegoUIKitSignaling.getInstance().addZIMEventHandler(this.listenerID, this.zimEventHandler);
            }
            catch (e) {
    ...
    async enableOfflinePush(enable) {
        // 申请通知权限
    ...
    addInvitation(invitees) {
        const _invitees = this.getUIKitUser(invitees);
        return ZegoUIKitSignaling.getInstance().addInvitation(_invitees);
    }
    cancelInvitation(config) {
        const { callID, invitees, notificationConfig } = config;
        return ZegoUIKitSignaling.getInstance().cancelInvitation(callID, invitees, notificationConfig);
    }
    // 被叫方对邀请做出回应数据上报
    incomingCallResponseReport(callID, action) {
    ...
    // 发起方对邀请回应数据上报
    outgoingCallResponseReport(callID, action, invitees) {
        const { state } = getAppState();
