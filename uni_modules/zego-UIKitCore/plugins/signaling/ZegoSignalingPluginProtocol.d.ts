...
export interface ZegoSignalingPluginProtocol extends ZegoPluginProtocol {
     ...
     */
    sendInvitation(invitees: string[], timeout: number, extendedData: string, notificationConfig?: ZegoSignalingNotificationConfig, mode?: ZIMCallInvitationMode): Promise<ZIMCallInvitationSentResult>;
    /**
     * 邀请其他用户加入呼叫邀请。
     * @param invitees 被邀请者的用户ID列表。
     ...
     */
    callingInvitation(invitees: string[], callID: string, extendedData: string, notificationConfig?: ZegoSignalingNotificationConfig): Promise<ZIMCallingInvitationSentResult>;
    /**
     * 取消已发出的邀请。
     ...
     */
    cancelInvitation(invitees: string[], invitationID: string, extendedData: string, pushConfig?: ZegoSignalingNotificationConfig): Promise<ZIMCallCancelSentResult>;
    /**
     * 拒绝邀请。
     * @param invitationID 邀请ID。
     * @param data 自定义数据。
     ...
     */
    setUsersInRoomAttributes(attributes: Record<string, string>, userIDs: string[], roomID: string): Promise<ZegoSignalingSetUsersInRoomAttributesResult>;
    /**
     * 查询房间内用户的属性。
     * @param roomID 房间ID。
     * @param count 查询数量。
     ...
     */
    updateRoomProperty(attributes: Record<string, string>, roomID: string, isForce: boolean, isDeleteAfterOwnerLeft: boolean, isUpdateOwner: boolean): Promise<ZIMRoomAttributesOperatedResult>;
    /**
     * 删除房间属性。
     * @param keys 属性键列表。
     * @param roomID 房间ID。
     ...
     * @param notificationConfig 通知配置。
     */
    callQuit(callID: string, extendedData: string, notificationConfig?: ZegoSignalingNotificationConfig): Promise<ZIMCallQuitSentResult>;
    /**
     * 结束呼叫邀请。
     * @param callID 呼叫邀请ID
     * @param extendedData 自定义数据
     * @param notificationConfig 通知配置。
     */
    callEnd(callID: string, extendedData: string, notificationConfig?: ZegoSignalingNotificationConfig): Promise<ZIMCallEndSentResult>;
}
